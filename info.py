...
DREAMCINEZONE_MOVIE_UPDATE_CHANNEL = int(environ.get('DREAMCINEZONE_MOVIE_UPDATE_CHANNEL', '-1002690909484'))  # Notification of those who verify will be sent to your channel
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002565101416'))  # Premium logs channel id
auth_channel = environ.get('AUTH_CHANNEL', '-1002507871737')  # Channel/Group ID for force sub (make sure bot is admin)
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1002518988143').split()]
...
AUTH_CHANNEL = [int(fch) if id_pattern.search(fch) else fch for fch in environ.get('AUTH_CHANNEL', '-1002185732095').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002627462219').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
...
DREAMCINEZONE_VERIFIED_LOG = int(environ.get('DREAMCINEZONE_VERIFIED_LOG', '-1002512328415'))  # Log channel id (make sure bot is admin)
...
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/All_new_and_old_collecton')
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/Yuvraaj233')
DREAMCINEZONE_MOVIE_UPDATE_CHANNEL_LNK = environ.get('DREAMCINEZONE_MOVIE_UPDATE_CHANNEL_LNK', 'https://t.me/DragonMax_Official_Updates')
OWNERID = int(os.environ.get('OWNERID', '6522113087'))  # Replace with the actual admin ID
